...
it('测试 输出到控制台', function () {
    ...
    log.log
        .location
        .text.yellow('log', '黄色未加粗');

    log.warn
        .location
        .text.magenta.bgCyan.bold('warn', '紫色粗体背景蓝色');

    log.error
        .location
        .text.green.underline('error', '绿色带下划线');
...
describe('测试 时间显示', function () {
    ...
    it('测试只显示日期', function () {
        expect(log.noTime.blue.format('2')).to.be([
            chalk.gray(`[${moment().format('YYYY-MM-DD')}]`),
            chalk.blue('2')
        ].join(' '));
    });
    ...
    it('测试不显示日期时间', function () {
        expect(log.noDatetime.blue.format('4')).to.be([
            chalk.blue('4')
        ].join(' '));

        expect(log.noDate.noTime.blue.format('5')).to.be([
            chalk.blue('5')
        ].join(' '));
    });
});
...
describe('测试 样式层', function () {
    ...
    it('测试多个样式层', function () {
        ...
        expect(log.red.bold.text.blue.bgGreenBright.text.yellow.underline.format(1, 2, 3))
            .to.be(log.text.red.bold.text.blue.bgGreenBright.text.yellow.underline.format(1, 2, 3));
    });
...
it('测试 chalk的方法', function () {
    const actual = log.noDatetime
        ...
        .bgRgb(1, 2, 3).bgHsl(1, 2, 3).bgHsv(1, 2, 3).bgHwb(1, 2, 3).bgHex('#DEADED').bgKeyword('orange').format(1);
